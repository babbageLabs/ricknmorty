      ...
      <section className="w-full">
        <Header>
          <div className="flex, flex-1 justify-center items-center text-center">
            <h3 className="text bg-center font-mono font-medium text-2xl">
              {character.name}
            </h3>
          </div>
        </Header>
            ...
            tabs={[
              {
                ...
                content: (
                  <div className="flex flex-col items-center">
                    <KV label="Gender">{character.gender}</KV>
                    <KV label="Status">{character.status}</KV>
                    <KV label="Locations">{character.location.name}</KV>
                  </div>
                ),
              },
              {
                  ...
                  <div>
                    <Grid>
                      <div
                        ...
                        className="flex flex-wrap justify-center p-2 episodes"
                      >
                        {character.episode.map((episode, i) => (
                          <EpisodeCard key={`episode-${i}`} url={episode} />
                        ))}
                      </div>
                    </Grid>
                  </div>
